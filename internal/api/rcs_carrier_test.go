...
// RCS declares a category, and every value it declares maps to a use case
// Airtel knows. The customer picks it: DLT's TRANSACTIONAL covers banking and
// OTP traffic, which Airtel separates, so deriving the use case from the DLT
// category guesses exactly where a wrong answer is silently auto-rejected.
func TestEveryRCSCategoryMapsToACarrierUseCase(t *testing.T) {
	for _, row := range []struct {
		category, useCase string
	}{
		{"MARKETING", "PROMOTIONAL"},
		{"UTILITY", "TRANSACTIONAL"},
		{"AUTHENTICATION", "OTP"},
	} {
		carrier := &stubRegistrar{vendor: "airtel", issued: "id-" + row.category}
		h := newCarrierHarness(t, carrier)
		tenant := h.newAccount("owner")
		templateID := h.rcsTemplate(tenant, "Cat "+row.category,
			[]string{"first_name"}, row.category)

		res := h.do(http.MethodPost,
			"/v1/templates/"+templateID.String()+"/carrier-registration", tenant.Token,
			map[string]any{"vendor": carrier.vendor})
		if res.Code != http.StatusOK {
			t.Fatalf("%s = %d, want 200; body = %s", row.category, res.Code, res.Body)
		}
		if carrier.sawSpec.UseCase != row.useCase {
			t.Errorf("%s submitted under use case %q, want %q",
				row.category, carrier.sawSpec.UseCase, row.useCase)
		}
	}
}

// No category means no use case, and there is no safe default: a promotional
// template under a transactional agent is auto-rejected by Airtel.
func TestAnUncategorisedRCSTemplateIsRefusedRatherThanGuessedAt(t *testing.T) {
	carrier := &stubRegistrar{vendor: "airtel", issued: "never"}
	h := newCarrierHarness(t, carrier)
	tenant := h.newAccount("owner")
	templateID := h.rcsTemplate(tenant, "Uncategorised", []string{"first_name"}, "")
